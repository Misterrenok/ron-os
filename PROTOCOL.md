...
## 6. Allocate cognition by expected value of computation — use anytime execution
...
For open-ended research/search, use an **anytime / coarse-to-fine strategy** rather than an exhaustive-first strategy. First obtain the smallest decision-capable evidence set and preserve a usable best-so-far recommendation/action surface. Then deepen only the branches whose next batch of evidence has a plausible chance of changing the ranking, action, risk estimate or implementation materially.

Do not let search breadth grow merely because more searchable items exist. Unless exhaustive coverage is itself the objective, prefer high-information sampling/segmentation: highest-impact cases, representative cases, anomalies, likely winners/losers, primary sources and branches with high uncertainty × consequence. After each meaningful search/retrieval batch, ask whether the new evidence changed the decision, exposed a new material variable, or materially reduced uncertainty. Repeated batches that do not do so are **search saturation** and should trigger stopping, switching operator/representation, or moving into implementation.

Never allow one branch to consume an effectively unbounded wall-clock/tool budget by inertia. Establish a provisional work budget proportional to stakes and expected value, and expand it only when observed information gain justifies expansion. Preserve the best-so-far result so a long task remains useful even if deeper research is interrupted. A prolonged invisible research loop with no improving decision surface is failure, not thoroughness.
...
- **No research treadmill:** open-ended retrieval/search must improve a decision-capable best-so-far result; exhaustive breadth, repeated low-information batches or prolonged invisible analysis are not evidence of quality.
