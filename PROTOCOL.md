...
A derived **current** claim inherits the weakest provenance of every material input. Never fill a missing current input from stale/planned evidence; if it can change the answer, keep the result `UNKNOWN`, conditional, threshold-based, or range-bounded until that input is verified.

## Runtime route
For current personal/project/app state: start at `BOOTSTRAP.md`, let it select direct/domain/cross-domain-global mode, then route through `references/domain-routing.md -> selected domain skills + exact owners -> live owners if mutable`. Read `CURRENT.md` only when the bootstrap-selected mode requires the cross-domain/global checkpoint; it is not a mandatory pre-read for ordinary domain recovery. Select the smallest complete union of primary and materially supporting domains. Memory, old chats, summaries, archived skill snapshots and exports are evidence only. Planned/prefilled/scheduled state is not real-world execution.
...
Before creating any temporary external resource, preflight its full lifecycle: verify cleanup is tool-executable under current authority; otherwise prefer an existing reusable resource or local/transactional isolation, and do not create persistent residue unless Ron explicitly accepts it before creation.
If assistant-created temporary residue remains after execution, the task is not `done`/`closed` unless that resource is deliberately promoted to a documented persistent role; blocked cleanup is an explicit unresolved blocker, never a silent handoff to Ron.
