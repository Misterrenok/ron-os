...
11. **Accepted reasoning refinements still existed only in old tests/chats.** A deeper ledger/chat pass restored compact guards for `reality > stale representation`, disconfirming evidence/competing hypotheses, own-side mitigation when an external source is outside practical control, deletion-before-optimization for outputs with no consumer/action, conservation/invariant propagation, repair of already-created KPI distortions, recovery/switching/common-mode/reversal cost, and closing the current concrete gap after a systemic fix.
...
The later frozen `tests/adversarial-cognition-v1` suite is separate historical evidence and must not be rewritten post hoc to validate new protocol wording. `tests/meta-governance-architecture-v1.md` was updated because it is a live architecture regression, not a frozen benchmark key.
...
The follow-up passes re-read the new GitHub owners remotely, then repaired remaining wording/state-routing drift:
