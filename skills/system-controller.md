...
* Deterministic non-choice follow-through may execute without another confirmation when it introduces no new user decision: qualifying evidence may flow through `system-evidence-followthrough:v1` into verified `quest.resolve`; a ledger-derived achievement may be unlocked when its existing policy checks pass; and exactly one gate-approved same-trajectory continuation may be created/focused after verified completion.
* `GIVE_QUEST`/`CREATE_QUEST`, `quest.focus`, recommended-window set/reschedule, attribute/skill/profile/shop configuration, `REDEEM`, `CANCEL` and acknowledgement actions remain **user-directed**, except for the narrowly bounded same-trajectory continuation explicitly allowed by `system-evidence-followthrough:v1`. An unambiguous request/choice is sufficient authorization for that exact internal action; a recommendation, hypothetical, complaint, status question or ambiguous mention is not.
...
* If every required objective can be verified, prepare and execute one atomic `quest.resolve` immediately, read back, then evaluate deterministic achievements. Do not ask for a redundant permission prompt.
