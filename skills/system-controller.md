...
* `CHALLENGE` is user-directed and writable only through `challenge.create` after Ron has unambiguously accepted the exact artificial deadline and exact recovery contract; direct `quest.create` Challenge and retrofitting an existing quest remain forbidden; engineering/maintenance capability activation does not itself authorize creating a Challenge for Ron.
