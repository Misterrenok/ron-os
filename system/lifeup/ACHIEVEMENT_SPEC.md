...
- Detection produces a deterministic candidate only. `achievement.unlock` remains a separate player-state mutation requiring exact permission under the normal System mutation gate.
...
Thresholds are cumulative. At a later checkpoint, every newly eligible not-yet-unlocked milestone may be proposed; the controller should not fabricate unlock events for old eligibility without exact permission.
