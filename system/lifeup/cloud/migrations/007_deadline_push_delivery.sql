-- Delivery infrastructure. Quest and notification truth remains in system_events.
...
-- Timing/Pressure v2 hardening for the same canonical PostgreSQL action gate.
-- The public mutation path remains system_apply_action(...). The private inner
-- function is implementation-only. In normal startup 005 recreates the current
-- Quest v2 gate before 007 runs, so the inner implementation is refreshed. If
-- 007 is re-run alone, the wrapper is not cloned into itself.
