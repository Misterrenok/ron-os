...
- A known blocker that is unchanged and still non-tool-executable is deferred, not re-selected as the primary slice; choose another materially useful executable slice or `NO-OP/PASS`. Retry it only when capability, authority, target state or evidence materially changes, and surface it to Ron only when it blocks the current outcome.
