...
6. OPEN architecture delta from the 2026-09-13 permission audit: remove redundant second-confirmation friction for bounded **internal** System actions. An unambiguous user System intent should authorize that exact internal action without another payload-confirmation prompt; deterministic verified `quest.resolve` and ledger-derived achievement unlocks are candidates for standing internal authorization. External writes stay under `PROTOCOL.md`; ambiguous intent must not mutate; attribute/skill/profile/shop configuration remains user-directed; redeem/cancel/ack require an unambiguous user choice; maintenance automation must not play for Ron or widen its own authority. No repo write-gate change has been applied because creation of the required Architecture Mode manifest was blocked by platform safety. Reused branch `system-fast-execution-v1` was only fast-forwarded to base `b9a2743d1201ada17a148bf29ddbb050e788952c`; it contains no candidate delta.
7. OPEN canonical wording drift: `BOOTSTRAP.md` still describes `projects/lifeup-system.md` with legacy wording that calls LifeUp a derived game/execution surface even though current System authority says LifeUp is retired/rollback-only. Fix this on the next valid Architecture Mode/control-plane change rather than silently editing bootstrap outside the architecture gate.
