...
**2026-09-20 novel total-value stress probe:** a precommitted, sealed counterfactual work/learning decision test scored **7 PASS / 3 PARTIAL / 0 hard fails**, which is an **overall FAIL under its frozen threshold** (required >=8 full PASS and <=2 PARTIAL). Misses localized to (1) Calendar projection vs observed execution provenance, (2) underdeveloped reject-both/continue-search challenger, and (3) ranking the current job without a refreshed full-compensation threshold despite acknowledging finance uncertainty. This does **not** yet justify a runtime architecture mutation; first localize whether existing provenance/alternative-generation rules already cover the defect and add the smallest falsifiable correction only if needed. Evidence: `tests/total-value-novel-counterfactual-v2-result-2026-09-20.md`.
