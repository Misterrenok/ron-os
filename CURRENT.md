...
**2026-09-20 targeted provenance + contra closure after Custom Instructions activation — PASS:** the fresh ordinary-chat frozen job-offer prompt passed C1–C5 with 0 hard fails H1–H4: stale Calendar 07:30 was rejected as current and no current hours/TL-per-hour were fabricated. A separate verified-input contra-test then supplied an explicit factual 07:30 schedule and all compensation inputs; the assistant correctly computed current/A/B weekly and monthly occupied hours plus cash-like hourly compensation, so the guard did not overcorrect into refusing valid arithmetic. The targeted stale-current-input / assumption-laundering regression is therefore CLOSED for the combined production configuration (active Custom Instructions + current Ron OS), not claimed universally. The first closeout write to `CURRENT.md` triggered the architecture change gate because `CURRENT.md` is a protected architecture surface; this bookkeeping closeout is being repaired through Architecture Mode without changing runtime reasoning semantics. Evidence: `tests/custom-instructions-activation-targeted-pass-2026-09-20.md` and `tests/custom-instructions-activation-contra-pass-2026-09-20.md`.
