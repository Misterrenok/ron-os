...
**2026-09-01 regression hardening after real failures:** no new runtime rule or Custom Instruction was added because the governing provenance/closeout rules already existed. Blind adversarial tests **T19 intra-owner provenance laundering** and **T20 automatic continuity responsibility** were added to `tests/adversarial-cognition-v1/`, with CI anchors in `tests/continuity_coverage_guard.py`. T19 rejects promoting summary/fallback/derived owner content to current/direct truth without stronger provenance; T20 rejects requiring Ron to say “remember/save/update CURRENT” for material Ron OS continuity capture. This is regression evidence/guarding, not a second mutable owner.
