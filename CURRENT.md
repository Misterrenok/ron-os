...
- Current prep architecture: Saturday evening boil 8 eggs. Sunday after library cook ~650 g raw chicken for Mon–Wed, the full ~700 g cooked-lentil weekly batch (refrigerate ~400 g, freeze ~300 g for Thu–Sat after cooling), and ~420 g dry pasta for Mon–Wed. Wednesday cook ~750 g raw chicken + ~420 g dry pasta for Thu–Sat and move the frozen lentils to the refrigerator. No oat-breakfast batch.
- Existing ~1.20-L lunch container and two cold packs are sufficient for launch design; afternoon chicken can travel in a food-safe resealable bag/wrap, and Mon–Sat 16:45 yogurt is moved to home dinner.
- Ron explicitly authorized both remaining Cronometer corrections on 2026-09-23: move the six 50 g yogurt rows (28.09–03.10) from snacks to dinner without amount changes, and correct the two sardine rows (30.09, 04.10) to the exact Dardanel whole-product layer with donor-only fish micronutrients scaled to ~85 g drained fish/can. The normal connector is not exposed in the current turn. A fallback Cronometer browser write was actually attempted on 2026-09-23 but stopped before mutation because that browser profile has no configured Cronometer credentials/session. **No live change occurred.** Permission is already granted; the only blocker is authenticated live access followed by write/read-back.
- Remaining non-PASS nutrition rows are only empirical/live: #20 first real prep/cleanup/storage fit, #25 first physical procurement/carrying route, #28 live Cronometer write/read-back, and #30 actual launch/adherence. Do not create new paper-analysis OPEN items without material new evidence.
- Day-1 target remains **2026-09-28** if Ron actually starts. Do not infer D3 purchase/intake, food purchase, training resumption, or adherence from planned rows.
...
**2026-09-20 operand-provenance execution v2:** the first closure passed structural CI but failed a fresh ordinary-chat replay: the answer rejected Calendar as truth yet still produced ~66.5 h/week by implicitly reusing Calendar-only 07:30. v2 therefore moves provenance checking into the total-value optimizer's arithmetic step: before present-tense calculation, material operands and their provenance must be identified; stale/planned/unknown operands cannot support a current point estimate. PR #66 merged and post-merge main continuity CI passed. Structural deployment is complete; one fresh ordinary-chat replay is still required before behavioral closure.

**2026-09-20 assumption-laundering closure v3:** the v2 fresh-chat replay still failed by explicitly saying it would *assume* stale Calendar-only 07:30 and then using ~66.5 h/week as current. The optimizer now forbids upgrading stale/planned evidence merely by labelling it an assumption: such values may be used only in an explicit counterfactual IF/scenario, and cannot feed the current ranking unless sensitivity shows the result is robust across plausible current values. PR #67 merged; candidate, promoted-candidate, PR and post-merge continuity CI passed. Structural deployment is complete; one fresh ordinary-chat replay remains required before behavioral closure.

**2026-09-20 direct-reasoning overlay v4 — BEHAVIORAL FAIL:** root-cause routing fix deployed structurally, but the fresh ordinary-chat replay still fabricated a current workload range (~55–67 h/week) despite the current start time being unspecified; it then used that range in current-hour comparisons. This means v4 does not behaviorally close the defect. Repeated failures across v1–v4 now point to the global activation layer: fresh chats are not proven to load repo BOOTSTRAP/PROTOCOL/optimizer. Issue #26 rollback is now the preferred next move: restore the prior Custom Instructions, then personal `Ron Work Protocol` if needed, and retest before making more repo-local optimizer changes.

**2026-09-20 minimal-CI provenance replay:** after instructing Ron to add a narrow global provenance paragraph to Custom Instructions, the frozen fresh-chat job scenario still returned a current-job range of ~54.7–66.5 h/week. Both endpoints depend on stale Calendar-only 07:30, so the targeted failure persisted; A↔B arithmetic remained valid. Because the assistant cannot independently inspect whether the user-side Custom Instructions change was saved/active, this is evidence of persistent failure but not a clean separation of non-activation vs non-compliance. Next controlled intervention: activate the existing BOOTSTRAP/PROTOCOL layer for consequential self-contained decisions, then rerun both the stale-input case and a verified-input contra case.

**2026-09-20 Custom Instructions activation update:** Ron directly confirmed he replaced the active Custom Instructions with the final combined version: current-state requests route to GitHub/BOOTSTRAP; consequential present-tense calculations preserve operand provenance; trivial self-contained requests stay direct; nontrivial/consequential self-contained decisions now explicitly route through BOOTSTRAP lean direct mode without forcing CURRENT/owners unless needed. This user-side activation change is now awaiting a fresh frozen-prompt behavioral replay.

**2026-09-20 targeted provenance + contra closure after Custom Instructions activation — PASS:** the fresh ordinary-chat frozen job-offer prompt passed C1–C5 with 0 hard fails H1–H4: stale Calendar 07:30 was rejected as current and no current hours/TL-per-hour were fabricated. A separate verified-input contra-test then supplied an explicit factual 07:30 schedule and all compensation inputs; the assistant correctly computed current/A/B weekly and monthly occupied hours plus cash-like hourly compensation, so the guard did not overcorrect into refusing valid arithmetic. The targeted stale-current-input / assumption-laundering regression is therefore CLOSED for the combined production configuration (active Custom Instructions + current Ron OS), not claimed universally. The first closeout write to `CURRENT.md` triggered the architecture change gate because `CURRENT.md` is a protected architecture surface; this bookkeeping closeout was repaired through Architecture Mode without changing runtime reasoning semantics. Evidence: `tests/custom-instructions-activation-targeted-pass-2026-09-20.md` and `tests/custom-instructions-activation-contra-pass-2026-09-20.md`.
...
- **2026-09-20 correction:** one of Ron's main long-term goals is **financial independence for autonomy**—being able to spend time on what he likes instead of working because money forces him to. **No profession, job title or industry is currently chosen as his desired endpoint.**
- The previously proposed commerce+analytics+automation stack and German Ausbildung occupations are **assistant-generated candidates only**, not Ron's career commitment. Ron's ~4 months Trendyol-only exposure remains the truthful baseline.
- Skill choices should preserve optionality, build transferable evidence and improve income/capital/autonomy while also generating preference-relevant experience.
...
- 2026-09-20 correction: Ron has no chosen profession; networking targets must follow the **specific route currently being tested**, not create a de facto career choice. Existing FISI/data/digitalization/e-commerce circles are dated candidate contexts only.
...
- **2026-09-23 continuity repair:** Ron's exact previously shared Güz timetable is recovered. Monday 11:00–13:00 online, Tuesday 12:00–17:00 on-site, Thursday 11:00–14:00 on-site, Friday 09:00–12:00 on-site and Saturday 11:00–12:00 online all overlap the current Mon–Sat 07:30–18:00 job; Sunday has online classes 10:00–11:00 and 12:00–14:00, Wednesday none shown. Therefore regular full attendance and the current job are structurally incompatible as scheduled. Anecdotes about rare attendance preserving study/residence remain **UNVERIFIED** and must not be treated as policy.
...
- Ron has already used about **two academic years of Turkish-language preparation** across Üsküdar + Topkapı. Official evidence supports treating the ordinary **Türkçe-hazırlık maximum as exhausted for planning purposes**; do not rely on another routine preparation year after transfer.
- This does **not** automatically block a future university change: Ron already has **C1 Turkish**. A Turkish-medium transfer/new admission may avoid preparation entirely if the target institution accepts his C1 evidence; exact acceptance is institution-specific and must be checked live.
- **2026-09-20 route map, not a chosen career:** Germany/Ausbildung is one serious route family under comparison; Turkish student/work-permit stabilization and other lawful employer-sponsored routes remain alternatives. Do not label any Ausbildung occupation or industry as Ron's selected endpoint.
...
- The strategy thread is **PAUSED, NOT CLOSED**. The weekly Topkapı timetable is now recovered and the schedule conflict with work is established. When resumed, the next evidence-gathering step is the real **course-level attendance/devamsızlık rules or records** and whether a selected-course/exam strategy can preserve the desired academic/legal option without pretending regular full attendance fits the job.
