...
**2026-09-20 targeted provenance replay after Custom Instructions activation — PASS:** a fresh ordinary-chat run of the exact frozen job-offer prompt passed C1–C5 with 0 hard fails H1–H4. The answer rejected stale Calendar 07:30 as current, did not compute current hours/TL-per-hour, used the correct meal-compensation threshold, preserved alternatives and action sequence, and kept A↔B arithmetic intact. This closes the targeted stale-current-input regression for the combined production configuration, not universally; a verified-input contra-test remains OPEN to rule out overcorrection.
