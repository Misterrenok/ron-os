...
**2026-09-20 direct-reasoning overlay v4:** root-cause audit found that the repeated fresh ordinary-chat provenance failures were not clean causal tests of v1-v3 optimizer edits because the self-contained direct path could bypass `PROTOCOL.md` and `skills/total-value-optimizer.md` entirely. Direct mode now preserves the low-overhead owner/CURRENT bypass for fully supplied tasks but still loads the lean reasoning layer for nontrivial/consequential decisions and the total-value overlay for materially multidimensional optimization. PR #68 merged; candidate, promoted-candidate, PR, post-merge continuity and LifeUp CI all passed. Structural routing deployment is complete; a fresh ordinary-chat replay remains required before behavioral closure.
